...
class Cart(object):
    def __init__(self, request):
        ...
        #получить корзину с текущей сессии
        cart = self.session.get(settings.CART_SESSION_ID)
        if not cart:
        # сохранить как пустую
    ...
    #update_quantity : Это логическое значение, которое указывает, требуется ли обновление количества с заданным количеством (True),
